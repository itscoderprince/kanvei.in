...
export async function GET(request, context) {
  try {
    ...
    // Optimization: Batch fetch all option images
    const optionIds = productOptions.map(opt => opt._id)
    const allOptionImages = await OptionImage.find({ optionId: { $in: optionIds } }).lean()

    // Create map for option images
    const optionImageMap = {}
    allOptionImages.forEach(imgDoc => {
      optionImageMap[imgDoc.optionId.toString()] = imgDoc.img
    })

    const optionsWithImages = productOptions.map(option => {
      return {
        ...option,
        images: optionImageMap[option._id.toString()] || []
      }
    })
