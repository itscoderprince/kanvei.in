    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        try {
          ...
          if (!user.password) {
            console.log(
              "User registered via OAuth, cannot login with credentials:",
              email,
            );
            return null;
          }
  ...
  callbacks: {
    ...
    async signIn({ user, account, profile }) {
      try {
        ...
        console.log(
          `📧 Sending ${notificationType.toLowerCase()} notification for ${loginType} login to:`,
          user.email,
        );
        ...
        try {
          ...
          console.log(
            `✅ ${notificationType} notification sent successfully for ${loginType} login`,
          );
        } catch (emailError) {
          console.error(
            `❌ Failed to send ${notificationType.toLowerCase()} notification:`,
            emailError,
          );
